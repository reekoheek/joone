...
public class NeuralNet implements NeuralLayer, NeuralNetListener, Serializable {
    ...
    protected void fastRun(int firstEpoch) {
        ...
        for (int epoch=epochs; epoch > 0 ; --epoch) {
            ...
            try {
				...
				log.warn("Running error", e);
				mon.fireNetStoppedError(e.getMessage());
				return;
			}
    ...
    protected void stepForward(Pattern pattern) {
        ...
        // hacked by reekoheek for the rest of layers if it is stop using the same pattern instead of null
        for (int ly=0; ly < layers; ++ly) {
        	if (ordLayers[ly].isInputLayer()) {
                ordLayers[ly].fwdRun(pattern);
        	} else {
