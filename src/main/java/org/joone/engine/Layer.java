...
import org.joone.exception.JooneException;
...
/**
 * The Layer object is the basic element forming the neural net. Primarily it
 * consists of a number of neurons that apply a transfer function to the sum of
 * a number of input patterns and convey the result to the output pattern. The
 * input patterns are received from connected input listeners and the
 * transformed results are passed to connected output listeners. The component
 * also handles learning by accepting patterns of error gradients from output
 * listeners, applying a reverse (inverse) transfer function and passing the
 * result to the input listeners. Layers execute their own Threads to perform
 * the perform the pattern conveyance, so that a network of Layers can operate
 * in a multi-threaded manner. The execution and termination of the Thread is
 * controlled by a Monitor object.
 */
public abstract class Layer implements NeuralLayer, Runnable, Serializable, Inspectable, LearnableLayer {
	...
	/**
	 * The monitor of the layer. Contains all parameters needed to the learning
	 * phase
	 */
	...
	/**
	 * Contains true if for the Layer must be used a Learner instead of a
	 * built-in learning algorithm. Set it in the constructor of any inherited
	 * class. Used by the getLearner method.
	 * 
	 * @see getLearner
	 */
	...
	/**
	 * Set of output values passed from this layer to connected OutputListeners
	 * durng the recall phase.
	 */
	protected transient double[] outs;

	/**
	 * Set of input values passed to this layer from connected InputListeners
	 * during the recall phase.
	 */
	protected transient double[] inps;

	/**
	 * Set of input error gradient values passed to this layer from connected
	 * OutputListenrs during the learning phase.
	 */
	protected transient double[] gradientInps;

	/**
	 * Set of output error gradient values passed from this layer to connected
	 * InputListenrs during the learning phase.
	 */
	...
	/**
	 * Adds a noise componentto the biases of the layer and to all the input
	 * connected synapses.
	 * 
	 * @param amplitude
	 *            the noise's amplitude in terms of distance from zero; e.g. a
	 *            value equal 0.3 means a noise range from -0.3 to 0.3
	 */
	 ...
	 * 
	 * @param amplitude
	 *            the amplitude of the applied noise
	 */
	 ...
	 */
	protected abstract void backward(double[] pattern) throws JooneRuntimeException;

	/**
	 * Copies one layer into another, to obtain a type-transformation from one
	 * kind of Layer to another. The old Layer is disconnected from the net, and
	 * the new Layer takes its place.
	 * 
	 * @param newLayer
		...
		for (int index = 0; (index < currentSize) && running; index++) {
			...
			if (tempListener != null) {
				...
				if (tPatt != null) {
					...
					if (step != STOP_FLAG)
						/*
						 * In case of a recurrent network, the layer could
						 * receive patterns with different sequence numbers. The
						 * stored sequence number is the higher one.
						 */
						if ((step < tPatt.getCount()) || (tPatt.getCount() == STOP_FLAG)) // The
																							// stop
																							// is
																							// guaranteed
							step = tPatt.getCount();
	 ...
	 * 
	 * @param pattern
	 *            the Pattern to pass to the output synapses
	 */
	protected void fireFwdPut(Pattern pattern) {
		...
		for (int index = 0; (index < currentSize) && running; index++) {
			tempListener = (OutputPatternListener) outputPatternListeners.elementAt(index);
			if (tempListener != null) {
				boolean loop = false;
				...
				if ((currentSize == 1) && getMonitor().isLearningCicle(pattern.getCount()) && !loop) {
	 ...
	 * 
	 * @param pattern
	 *            the Pattern to pass to the input listeners
	 */
	 ...
	 * 
	 * @param aPattern
	 *            the pattern holding a different size than the layer (dimension
	 *            of neurons is not in accordance with the dimension of the
	...
	protected void adjustSizeToFwdPattern(double[] aPattern) {
		...
		log.warn("Pattern size mismatches #neurons. #neurons in layer '" + getLayerName() + "' adjusted [fwd pass, "
				+ myOldSize + " -> " + getRows() + "].");
	}
	...
	protected void adjustSizeToRevPattern(double[] aPattern) {
		...
		int myOldSize = getRows();
		setRows(aPattern.length);
		log.warn("Pattern size mismatches #neurons. #neurons in layer '" + getLayerName() + "' adjusted [rev pass, "
				+ myOldSize + " -> " + getRows() + "].");
	}
	...
	protected abstract void forward(double[] pattern) throws JooneRuntimeException;
	...
	/**
	 * The core running engine of the layer. Called from the method
	 * <CODE>start()</CODE>
	 * 
	 * @throws JooneRuntimeException
	 */
	public void run() throws JooneRuntimeException {
		...
		while (running) {
			...
			try {
				...
				String msg = "JooneException thrown in run() method." + jre.getMessage();
				log.error(msg);
				running = false;
			...
			if (learning && running) {
				...
				try {
					...
					// Added for some unsupervised learning algorithm (See
	...
	/**
	 * Sets the Vector that contains all the input listeners. Can be useful to
	 * set the input synapses taken from another Layer
	 * 
	 * @param newInputPatternListeners
	 *            The vector containing the list of input synapses
	 */
	public synchronized void setAllInputs(Vector newInputPatternListeners) {
		...
		if (inputPatternListeners != null)
			for (int i = 0; i < inputPatternListeners.size(); ++i)
				this.setInputDimension((InputPatternListener) inputPatternListeners.elementAt(i));
		notifyAll();
	}

	/**
	 * Sets the Vector that contains all the input listeners. It accepts an
	 * ArrayList as parameter. Added for Spring Can be useful to set the input
	 * synapses taken from another Layer
	 * 
	 * @param newInputPatternListeners
	 *            The vector containing the list of input synapses
	 */
	...
	/**
	 * Sets the Vector that contains all the output listeners. Can be useful to
	 * set the output synapses taken from another Layer
	 * 
	 * @param newOutputPatternListeners
	 *            The vector containing the list of output synapses
	 */
	public void setAllOutputs(Vector newOutputPatternListeners) {
		...
		if (outputPatternListeners != null)
			for (int i = 0; i < outputPatternListeners.size(); ++i)
				this.setOutputDimension((OutputPatternListener) outputPatternListeners.elementAt(i));
	}

	/**
	 * Sets the Vector that contains all the output listeners. It accepts an
	 * ArrayList as parameter. Added for Spring Can be useful to set the output
	 * synapses taken from another Layer
	 * 
	 * @param newOutputPatternListeners
	 *            The vector containing the list of output synapses
	 ...
	 * 
	 * @param newBias
	 *            The Matrix object containing the biases
	 */
	...
	/**
	 * Sets the dimension of the layer. Override to define how the internal
	 * buffers must be sized.
	 */
	protected abstract void setDimensions();

	/**
	 * Sets the dimension of the listener passed as parameter. Called after a
	 * new input listener is added.
	 * 
	 * @param syn
	 *            the listener to be affected
	 */
	 ...
	 * 
	 * @param vect
	 *            the Vector of pattern listeners
	 * @param mon
	 *            the Monitor to be set
	 */
	...
	/**
	 * Sets the dimension of the listener passed as parameter. Called after a
	 * new output listener is added.
	 * 
	 * @param syn
	 *            the OutputPatternListener to affect
	 */
	 ...
	 * 
	 * @param newRows
	 *            The number of the neurons contained in the Layer
	 */
			...
			if (myThread == null) {
				...
				if (inputPatternListeners != null) {
					if (checkInputEnabled()) {
						// If all the input synapses are disabled, the layer
						// doesn't start
						running = true;
						...
						String msg = "Can't start: '" + getLayerName()
								+ "' has not input synapses connected and/or enabled";
						log.error(msg);
						throw new JooneRuntimeException(msg);
					}
				} else {
					String msg = "Can't start: '" + getLayerName() + "' has not input synapses connected";
					log.error(msg);
					throw new JooneRuntimeException(msg);
				}
			}
	...
	protected void sumBackInput(double[] pattern) {
		...
		try {
			...
			log.warn(getLayerName() + " gradInps.size:" + gradientInps.length + " pattern.size:" + pattern.length
					+ " x:" + x);
		}
	}
	 ...
	 * 
	 * @param out
	 *            the output stream to write this layer to
	 * @throws IOException
	 */
	...
	/**
	 * This method is useful to serialize only the vector's elements that don't
	 * implement the Serialize interface, only when the Monitor.isExporting
	 ...
	 * 
	 * @param in
	 *            the input stream serialized version
	...
	/**
	 * Get check messages from listeners. Subclasses should call this method
	 * from thier own check method.
	 * 
	 * @see NeuralLayer
	...
	public TreeSet check() {
		...
		if ((inputPatternListeners == null) || (inputPatternListeners.size() == 0)) {
			checks.add(new NetCheck(NetCheck.FATAL, "Layer has no input synapses attached.", this));
		}
	...
	/**
	 * Method to help remove disused references quickly when the layer goes out
	 * of scope.
	 * 
	 * @see Object#finalize()
	 * @throws Throwable
	...
	/**
	 * Determine whether this layer has an input synapse attached that is a step
	 * counter.
	 * 
	 ...
	 * Also checks the attached listeners of OutputSwitchSynapses. Also checks
	 * for loopback condition. All connected synapses must be of this type.
	 * 
	 * @param outputListeners
	 *            Vector to check.
	...
	public void join() {
		try {
			...
			/*
			 * As we cannot synchronize this method, we could get a
			 * NullPointerException on calling myThread.join()
			 */
		}
	}
	 ...
	 * /*********************************************************
	 * 
	 * /** This method serves to a single forward step when the Layer is called
